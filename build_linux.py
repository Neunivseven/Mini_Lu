...
""",
    ...
    readme.write_text(
        ...
        "\n"
        "【数据存放位置（重要）】\n"
        "  聊天记录、笔记、API 配置等用户数据保存在:\n"
        "    ~/.local/share/Mini_Lu/  (data/ 与 config/)\n"
        "  更新/覆盖安装本程序不会丢失这些数据。\n"
        "  首次启动会自动把旧版安装目录里的数据迁移过去。\n"
        "  可用环境变量 MINI_LU_HOME 自定义该目录。\n"
        "\n"
        "【模型 / API】\n"
        "  右键形象 →「模型设置…」；或编辑 ~/.local/share/Mini_Lu/config/models.local.yaml\n"
        "  含 API Key 的文件请勿随意发给他人。\n"
        "\n"
        "【聊天】\n"
        "  助手回复支持 Markdown 表格/标题（需打包时含 markdown 库）。\n"
        "\n"
        "PDF：内置 PyMuPDF（config/doc_parsers.yaml）。\n"
        "工作区：~/.local/share/Mini_Lu/config/workspace.yaml。\n"
