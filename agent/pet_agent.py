...
DEFAULT_AGENT_SYSTEM = (
    ...
    "了解项目/接口/类结构时：先 glob_files 找源文件，再对关键头文件/模块调用 "
    "read_outline 或 list_symbols（tree-sitter）；需要某符号全文用 read_symbol。"
    "禁止用 list_directory + 整文件 read_file 当「读结构」的默认路径。\n"
    "1) 定位：glob_files / grep_files / read_outline / list_symbols。"
    "禁止一上来整文件 Read。\n"
    "2) 跨文件关系（默认 TSA）：先 codegraph_status；索引空/过期则 index_codebase(mode=incremental，可限 max_files)；"
    "再 find_callers / find_callees。不要默认 full 全库重索。\n"
    ...
    "4) 修改：一律优先 edit_file 精确替换；删除则 new_string 为空。禁止为小改动 write_file。\n"
